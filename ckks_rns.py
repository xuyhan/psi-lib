...
class RNSParams:
    def __init__(self, N: int, L: int, ps: List[int], qs: List[int]):
        self.N = N
        self.L = L
        self.ps = ps
        self.qs = qs
...
class Gen:
    ...
    @staticmethod
    def gen_a(n: int, m: int) -> Poly:
        coeffs = [0] * (n + 1)

        for i in range(n + 1):
            coeffs[i] = random.randint(0, m)

        return Poly.from_list(coeffs, gens=x)

    @staticmethod
    def gen_a_rns(n: int, ms: List[int]) -> List[Poly]:
        polys = []

        for m in range(len(ms)):
            polys.append(Gen.gen_a(n, ms))
...
class KeyGen:
    ...
    @staticmethod
    def RNSGen(params: Params):
        N = params.N

        s = Gen.gen_key(N)
        e = Gen.gen_error(N)
        a = Gen.gen_a_rns(N, params.qs)

        b = [(-a_i * s + e) % params.poly_mod for a_i in a]

        a_prime = Gen.gen_a(N, params.ps + params.qs)
        e_prime = Gen.gen_error(N)
        ...
        b_prime_1 = [
            round_poly((-a_prime[i] * s + e_prime), params.poly_mod, params.ps[i])
            for i in range(k)
        ]

        b_prime_2 = [
            round_poly((-a_prime[k + i] * s + e_prime + params.P * s * s), params.poly_mod,params.qs[i])
            for i in range(l)
        ]
        ...
        sk = (1, s)
        pk = [(b[i], a[i]) for i in range(l)]
        ek = [(b_prime[i], a_prime[i]) for i in range(k + l)]
...
N = 16
params = Params(N, p=16, p_0=1, L=3)
...
p_raw = np.array([10 * i for i in range(N // 2)])
q_raw = np.array([.5 * i for i in range(N // 2)])
